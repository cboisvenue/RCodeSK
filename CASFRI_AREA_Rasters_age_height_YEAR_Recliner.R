...
yearsNA <- c(1984:2012,NA)
sub_df <- data.frame(yearsNA, 0)
age_reclass <- subs(age_perYr, sub_df, by=1, which=2, subsWithNA=FALSE)
age1_mask <- age_reclass * StudyArea
...
writeRaster(age_reclass, file.path(paste(timeVARIANT, "age1" ,sep="")),
            format='GTiff', bylayer=TRUE, datatype='INT2U', overwrite=TRUE)
writeRaster(age1_mask, file.path(paste(intermediate_input, "age1_mask" ,sep="/")),
            format='GTiff', bylayer=TRUE, datatype='INT2U', overwrite=TRUE)
...
site_productivity_table <- data.frame(old=c(1:7), update=c(1,2,1,2,0,3,3))
beginCluster(30)
site_productivity <- subs(soil_moist_regime, site_productivity_table, by=1, which=2, subsWithNA=TRUE)
writeRaster(site_productivity, file.path(paste(intermediate_input, "site_productivity" ,sep="/")),
            format='GTiff', datatype='INT2U', overwrite=TRUE)
