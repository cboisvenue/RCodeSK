...
startTime <- Sys.time()
PaFMA_30_temp <- rasterize(PaFMA, template30, field=1, na.rm=TRUE)
endTime <- Sys.time()
...
PaFMA_30_temp4 <- projectRaster(PaFMA_30_temp3, template30)

# Set up Reference layer for BatchCrop
Crop30 <- PaFMA_30_temp4

#Write Study area raster to 'reference' directory (change name to reflect forest district)
setwd("H:/saskatchewan/data/reference")
writeRaster(Crop30, "Crop30_PaFMA_test.tif", overwrite=TRUE, format='GTiff', datatype='INT2U')
